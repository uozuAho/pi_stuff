...
DELETE_SOURCE_AFTER_SCAN=yes
# dvr-scan outputs small files, even if no motion was detected
DELETE_OUTPUTS_SMALLER_THAN=600k
...
for raw_vid in $VIDEOS_DIR/*.h264; do
    ...
    if [[ $DELETE_SOURCE_AFTER_SCAN == "yes" ]]; then
        rm $raw_vid
    fi
    if [[ $DELETE_OUTPUTS_SMALLER_THAN != "" ]]; then
        find $out_path -size -$DELETE_OUTPUTS_SMALLER_THAN -delete
    fi
done
